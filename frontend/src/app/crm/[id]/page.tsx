...
export default function LeadDetailsPage() {
  ...
  const handleGeneratePdf = async () => {
    ...
    // Simulação da chamada backend
    setTimeout(() => {
      setIsGeneratingPdf(false);
      alert("Proposta PDF gerada com sucesso! O download iniciará em instantes (Simulado via Backend /api/proposals).");
    }, 2000);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-100">
                  ...
                  <button 
                    ...
                    className="w-full bg-primary text-white font-bold py-3 rounded-lg mt-8 flex items-center justify-center gap-2 shadow-lg shadow-primary/20 disabled:opacity-50 group"
                  >
                     {isGeneratingPdf ? <Loader2 className="h-5 w-5 animate-spin" /> : <Download className="h-5 w-5" />}
                     {isGeneratingPdf ? "Gerando..." : "Gerar Proposta PDF"}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                { title: "Ligar para João", date: "Hoje, 14:00" },
                { title: "Enviar Proposta", date: "Amanhã, 09:00" },
              ].map((task, i) => (
          ...
          <div className="bg-gray-900 text-white rounded-xl p-6 shadow-xl">
             ...
             <div className="flex items-center gap-4 mb-4">
               ...
               <p className="text-xs text-gray-400">Probabilidade alta de fechamento.</p>
             </div>
